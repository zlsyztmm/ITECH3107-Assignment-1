...
public class ClassActivity extends Activity implements CallBack {
            ...
            public void run() {
                ...
                recyAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
                    ...
                    public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                        ...
                        bui.setTitle("是否删除该课程");

                        bui.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Toast.makeText(ClassActivity.this, "已删除", Toast.LENGTH_SHORT).show();
                                Course item = (Course) adapter.getItem(position);
                                new Thread() {
                                    @Override
                                    public void run() {
    ...
    public static class ProRecyAdapter extends BaseQuickAdapter<Course, ProRecyAdapter.ViewHo> {
        ...
        protected void convert(ProRecyAdapter.ViewHo helper, Course item) {
            helper.numTextView.setText("课程号:" + item.num);
            helper.nameTextView.setText("课程名字:" + item.name);
            helper.startTextView.setText("开始时间:" + item.startTime);
            helper.endTextView.setText("结束时间:" + item.endTime);
